...
Modev Suite is a small collection of standalone HTML files that work together as one app. Drop them into any folder, open `index.html`, and you get a responsive-design previewer, a full code editor with GitHub integration, a GitHub/Vercel publishing workflow, and a settings panel — all running as local files with zero setup.

---

## Quick start

1. Open **`index.html`** — everything lives behind this one entry point.
...
Each tool is its own HTML/CSS/JS trio instead of one large inline file — cleaner to navigate, same iframe-per-tool architecture underneath:

```
index.html / hub.css / hub.js                — the shell: tabs, splash, debug console
device-lab.html / .css / .js                   — responsive preview
github-uploader.html / .css / .js              — GitHub tab (repo/publish/collaborators)
code-editor.html / .css / .js                  — Code tab (module script — uses ES imports)
settings.html / .css / .js                     — Settings tab
console-bridge.js                              — shared: forwards each tool's console/errors to the hub's debug console (auto-detects which tool it's in from the filename)
icons.js                                       — shared inline-SVG icon set + auto-render observer
manifest.json, icons/                          — PWA manifest and app icons
```

The hub still loads each tool into an iframe and fully unloads it before loading the next — that's what keeps memory low on a phone, and splitting files doesn't change it. A true single-page version (one file, tab-switching, no iframes) was considered and intentionally not built, since it would mean every tool's memory stays resident at once — the opposite of what fixed the original crashing.
